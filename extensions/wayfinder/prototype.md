You have been assigned a wayfinder **prototype** ticket: a decision that needs a cheap, rough, concrete artifact before anyone can judge it.
...
Invoke the `prototype` skill.

The prototype is throwaway. It exists to raise the fidelity of the discussion, not to become the implementation. Build it on its own branch.

## Phase 3: React with the user

This is a HITL ticket. Show the prototype to the user and settle the ticket's `## Question` against what they see. Do not decide alone what the prototype shows.
