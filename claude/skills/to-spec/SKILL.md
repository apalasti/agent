...
Use the read tool to load `~/.claude/skills/issue-tracker.md` at the start of every run.

0b. Load the map, if there is one

If `.scratch/<effort-slug>/MAP.md` exists, this spec is the destination of a wayfinder map, and its decisions are on disk rather than in your context - they were resolved in sessions you never saw. Read `MAP.md` and the full body of every closed ticket before writing anything. Then:
...
- The map's **Out of scope** carries into the spec's Out of scope, near-verbatim
