...
The destination varies per effort, and naming it is the first act of charting: it shapes every ticket. Most often here it's a spec to hand to `to-spec`; it might also be a decision to lock before planning starts, or a change made in place like a data-structure migration.

Before doing anything else, use the read tool to load `~/.claude/skills/issue-tracker.md` in full — its **Wayfinding operations** section defines where the map, tickets, blocking, and the frontier physically live.

## Where this sits in the workflow

Wayfinder runs **before** `to-spec`, and only when the idea won't fit in one session:

```
loose idea → wayfinder map (decisions, HITL, one ticket per session)
           → to-spec → spec.md
           → to-tickets → .scratch/<effort>/issues/
           → /issue (execution, one issue per session)
```

If the way to the destination is already clear enough to hold in one head, skip this skill and go straight to `to-spec`.

Wayfinder tickets live in `tickets/`, never in `issues/`. The `/issue` command scans `issues/` and will try to TDD-implement anything it finds there — a decision ticket landing in that directory is a bug, not a shortcut.
...
Each ticket is a file in `tickets/`, and its body is the question, sized to **one session** (roughly one 100K token agent context):
...
- **`grilling`** (HITL) — Conversation. The default case. Invoke the `grilling` skill.
...
The **Out of scope** section carries straight into the spec's own Out of scope section when the map is handed off.
...
**Context pressure means the ticket was too big — split it, don't resume it.** This inverts the implementation loop, which aborts near the context limit, writes a handoff via `issue-handoff`, and resumes the same issue in a fresh session. That works because implementation is AFK. Wayfinding is mostly HITL, and respawning mid-grilling drops the human into a conversation with no memory of the last hour of dialogue. So when a ticket runs long: record what _is_ settled as its resolution, close it, and create a follow-up ticket for the remainder.
...
1. Invoke `to-spec`. It reads `MAP.md` and every closed ticket, so the decisions carry over without re-interviewing the user; the interviewing already happened, ticket by ticket.
2. Then `to-tickets` as normal, and the effort leaves the map for `issues/`.

Leave the map and its tickets in place. They're the primary sources behind the spec, and the spec links back to them.
